...
export function setupNoteHandlers() {
  ipcMain.handle(
    'note:create',
    ipcHandler(async (_event, req: NoteCreateRequest) => {
  ...
  ipcMain.handle(
    'note:read',
    ipcHandler(async (_event, req: NoteReadRequest) => {
  ...
  ipcMain.handle(
    'note:update',
    ipcHandler(async (_event, req: NoteUpdateRequest) => {
  ...
  ipcMain.handle(
    'note:delete',
    ipcHandler(async (_event, req: NoteDeleteRequest) => {
  ...
  ipcMain.handle(
    'note:list',
    ipcHandler(async (_event, req: NoteListRequest) => {
      const { vaultPath } = req;
  ...
  ipcMain.handle(
    'note:rename',
    ipcHandler(async (_event, req: NoteRenameRequest) => {
  ...
  ipcMain.handle(
    'note:get-title',
    ipcHandler(async (_event, req: NoteGetTitleRequest) => {
      const { notePath } = req;
  ...
  ipcMain.handle(
    'note:get-relative-path',
    ipcHandler(async (_event, req: NoteGetRelativePathRequest) => {
      const { notePath, vaultPath } = req;
